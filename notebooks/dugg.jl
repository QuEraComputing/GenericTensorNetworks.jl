...
# ╔═╡ c37be80e-e9ba-11ec-30ea-43b3344b86ac
using GenericTensorNetworks

# ╔═╡ cc58e2d2-1552-405d-9555-0e5dc31c47ab
using UnitDiskMapping, Graphs

# ╔═╡ b6d1e7b1-b40e-4166-94f1-f8eea3a01a7b
using Luxor

# ╔═╡ 5faa573e-c3d8-48b3-a1cd-fd86f3f5b9b6
using MathTeXEngine

# ╔═╡ cccc014d-b5e9-4e31-9860-af1b4f052e8e
locs = [(0, 0), (0, 1), (0, 2), (0, 3), (1,1), (1,3), (1,4), (2, 0), (2,2), (2,4), (3, 0), (3, 1), (3,2), (3,3), (4,0), (4,2), (4,4)]

# ╔═╡ 1c4f6598-630d-4db2-b571-033682094ce8
graph = unit_disk_graph(locs, 1.5)

# ╔═╡ 6770c854-b595-4073-9ce9-6a9c96eafa30
show_graph(graph; locs=[(j, -i) for (i, j) in locs], texts=fill("", length(locs)))

# ╔═╡ 694628f3-27b3-493a-a787-dc95fcce8163
res1 = solve(IndependentSet(graph), SingleConfigMax())

# ╔═╡ 48cf19bd-6485-4b7e-b6a6-b9bb90841f22
show_graph(graph; locs=[(j, -i) for (i, j) in locs], texts=fill("", length(locs)), vertex_colors=[iszero(c) ? "white" : "red" for c in res1[].c.data])

# ╔═╡ bb39a1e8-8dca-493c-8c13-f022cdf6afe1
res2 = solve(IndependentSet(graph), ConfigsMax())

# ╔═╡ 7d9b39df-44f1-42a4-80aa-4cbf07d9f480
show_gallery(graph, (1,4); locs=[(j, -i) for (i, j) in locs], texts=fill("", length(locs)), vertex_configs=res2[].c.data, image_size=5)

# ╔═╡ d2b24ea3-c6d5-4eda-b6ed-e78d61cb67f7
res3 = solve(IndependentSet(graph), GraphPolynomial())

# ╔═╡ 8da609ba-17b3-4eeb-8819-afc7078eb339
grid = map_graph(smallgraph(:petersen)).grid_graph

# ╔═╡ 55311eab-433a-432b-a5b9-a855dd0f90e8
locs2 = findall(!isempty, grid.content)

# ╔═╡ a61154ab-1b4a-43b7-9c03-522b973dbb4f
show_graph(SimpleGraph(grid); locs = [(ci.I[2], ci.I[1]) for ci in locs2], texts=fill("", length(locs2)), vertex_colors=fill("black", length(locs2)))

# ╔═╡ 0c4db665-4c1d-4754-802e-2866d69acc75
res_petersen = solve(IndependentSet(SimpleGraph(grid)), SingleConfigMax())[]

# ╔═╡ 65b665c6-3ec9-43ef-871c-d890dd0a4680
show_graph(SimpleGraph(grid); locs = [(ci.I[2], ci.I[1]) for ci in locs2], texts=fill("", length(locs2)), vertex_colors=[iszero(c) ? "white" : "red" for c in res_petersen.c.data], image_size=22)

# ╔═╡ 0459e421-f51f-4da6-a194-42653e5f2dfc
keys(Luxor.Colors.color_names)

# ╔═╡ 810e913f-a6f9-4509-8c03-95f90fe7499e
function draw_solution(yellows, greens)
	rad = 10
	centers = [Point(y*50, x*50) for (x, y) in locs]
	origin(Point(minimum(getfield.(centers, :x)) + 2*rad, minimum(getfield.(centers, :y)) + 2*rad))
	# show nodes
	#setcolor("white")
	#for p in centers
	#	circle(p, rad, :fill)
	...
	for i=0:4
		for j=0:4
			if (i, j) ∉ locs
				# show cross
				line(Point(j*50-rad, i*50-rad), Point(j*50+rad, i*50+rad), :stroke)
				line(Point(j*50-rad, i*50+rad), Point(j*50+rad, i*50-rad), :stroke)
			end
		end
	end
	...
	# 	a, b = Point(centers[e.src]...), Point(centers[e.dst]...)
	# 	nints, ip1, ip2 =  intersectionlinecircle(a, b, a, rad)
	# 	a_ = ip1
	# 	nints, ip1, ip2 =  intersectionlinecircle(a, b, b, rad)
	# 	b_ = ip2
	# 	line(a_, b_, :stroke)
	# end

	graph = square_lattice_graph(ones(Bool, 5, 5))
	_centers = [Point((ci.I[2]-1)*50, (ci.I[1]-1)*50) for ci in CartesianIndices((5, 5))]
	for e in edges(graph)
		a, b = Point(_centers[e.src]...), Point(_centers[e.dst]...)
		nints, ip1, ip2 =  intersectionlinecircle(a, b, a, rad)
		a_ = ip1
		nints, ip1, ip2 =  intersectionlinecircle(a, b, b, rad)
		b_ = ip2
	...
	# show nuts
	setcolor(sethue("yellow")..., 0.2)
	for (i, j) in yellows
		circle(50*j, 50*i, 44, :fill)
	end
	setcolor(sethue("limegreen")..., 0.2)
	for (i, j) in greens
		circle(50*j, 50*i, 44, :fill)
	end
end

# ╔═╡ d000c7bb-424e-4e30-b366-74d2303d0333
Luxor.get_current_color() == Luxor.Colors.color_names["limegreen"]

# ╔═╡ 3ced1a69-2a4d-4942-a72d-5d668844d2a5
@drawsvg draw_solution([(1,1)], [(2, 2)]) 500 250

# ╔═╡ 76473c61-dbc6-4493-afb2-930ad3fd98af
@drawsvg draw_solution([(1,1)], [(1, 3)]) 500 250

# ╔═╡ d9f3717b-8983-429d-8dbf-cd1fe505bcec
function draw_illustrate()
	draw_solution([(1, 6)], [])
	setcolor("black")
	text(L"\sqrt{3}", 350, 50)
end

# ╔═╡ ff14b193-5da0-447e-a17d-9568e5e86482
@drawsvg draw_illustrate() 500 250

# ╔═╡ 9726e405-b308-4b33-83d2-957a2e434e06
let
@drawsvg begin
	rad = 10
	setcolor("black")
...
# ╔═╡ 1b8c61b2-45e2-439d-9f66-fd9b87bffa88
@drawsvg let
origin()
background("grey6")
fontsize(26)
...
manifest_format = "2.0"
project_hash = "77fed6c30d30ca45eeea5d8fafc46db756399cdb"
...
git-tree-sha1 = "b2771fa64c9501731b29daba91e653c6fae80937"
...
deps = ["Graphs"]
git-tree-sha1 = "4aa41c61ed79b9e7394260613d5ad938fdf0cc48"
