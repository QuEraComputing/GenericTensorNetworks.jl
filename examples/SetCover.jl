...
# Given a set of elements ``\{1,2,...,n\}`` (called the universe) and a collection ``S`` of {\displaystyle m}m sets whose union equals the universe, the set cover problem is to identify the smallest sub-collection of {\displaystyle S}S whose union equals the universe.
