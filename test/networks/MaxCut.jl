...
@testset "graph utils" begin
    ...
    # weighted
    ws = collect(1:ne(g))
    gp = max_cut_network(g; edge_weights=ws)
...
@testset "MaxCut" begin
    ...
    @test graph_polynomial(max_cut_network(g), Val(:polynomial))[] == Polynomial([2,2,4,12,10,2])
    @test graph_polynomial(max_cut_network(g), Val(:finitefield))[] == Polynomial([2,2,4,12,10,2])
end
...
@testset "enumerating - max cut" begin
    ...
    code = max_cut_network(g; optimizer=GreedyMethod())
...
@testset "fix vertices - max cut" begin
    ...
    problem = max_cut_network(g, fixedvertices=fixedvertices)
...
@testset "vertex weights" begin
    ...
    gp = max_cut_network(g; edge_weights, vertex_weights)
