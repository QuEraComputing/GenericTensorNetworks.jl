...
@testset "enumerating - matching" begin
    ...
    code = matching_network(g; optimizer=GreedyMethod(), fixedvertices=Dict())
    res = solutions(code, CountingTropicalF64; all=true)[]
    @test res.n == 5
    @test length(res.c.data) == 6
    code = matching_network(g; optimizer=GreedyMethod(), fixedvertices=Dict((1,2)=>1))
...
@testset "match polynomial" begin
    ...
    @test graph_polynomial(matching_network(g), Val(:polynomial))[] == Polynomial([1,7,13,5])
    g = smallgraph(:petersen)
    @test graph_polynomial(matching_network(g), Val(:polynomial))[].coeffs == [6, 90, 145, 75, 15, 1][end:-1:1]
end

@testset "weighted matching" begin
    g = smallgraph(:petersen)
    problem = matching_network(g; weights=fill(2, 15))
